...
if [[ ( -f "$FLAG_FILE" && "$RAM_USAGE_INT" -le 90 )  &&  ( -f "$FLAG_FILE" && "$CPU_USAGE_INT" -le 95 ) ]]; then
...
rm $FLAG_FILE
curl -X POST -H 'Content-type: application/json' --data "$object_right" https://hooks.slack.com/services/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
exit
fi
if [[ ( ! -f "$FLAG_FILE" && "$RAM_USAGE_INT" -ge 90 )  ||  ( ! -f "$FLAG_FILE" && "$CPU_USAGE_INT" -ge 95 ) ]]; then
...
touch $FLAG_FILE
curl -X POST -H 'Content-type: application/json' --data "$object_wrong" https://hooks.slack.com/services/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
exit
fi
